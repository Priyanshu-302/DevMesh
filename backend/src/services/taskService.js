...
/**
 * Execute Follow-up Task on an existing Task (in-place)
 */
const executeFollowUpTask = async (taskId, followUpPrompt) => {
  // Find the task
  const task = await Task.findById(taskId).populate("workspace");
  if (!task) {
    const error = new Error("Task not found");
  ...
  if (!workspace) {
    const error = new Error("Workspace not found for this task");
    error.statusCode = 404;
    throw error;
  }

  // Check workspace ingestion status
  if (workspace.ingestionStatus !== "completed") {
    const error = new Error(
      `Task execution rejected: Workspace ingestion status is '${workspace.ingestionStatus}'. Must be 'completed'.`,
    );
    error.statusCode = 400;
    throw error;
  }

  // Prepend follow-up prompt to task requestText history
  const previousPrompt = task.requestText;
  task.requestText = `${previousPrompt}\n\n[Follow-up Command]: ${followUpPrompt}`;
  task.status = "in-progress";
  task.error = null;
  task.finalCode = "";
  await task.save();

  // 1. Clear previous logs for this task in database
  await AgentLog.deleteMany({ task: taskId });

  // 2. Broadcast status reset to web socket room
  broadcastToTaskRoom(taskId.toString(), "task_status_updated", { status: "in-progress" });

  // 3. Execute agent pipeline asynchronously
  process.nextTick(async () => {
    try {
      logger.info(`Running agent pipeline for follow-up Task ${task._id}...`);

      // Prepend previous prompts as context to the AI model
      const agentRequestText = `Context from previous conversation/task:\n` +
      ...
      logger.error(
        `Task pipeline execution failed for follow-up task ${task._id}: ${error.message}`,
      );
      task.status = "failed";
      task.error = error.message;
      await task.save();
...
module.exports = { createAndExecuteTask, getTaskById, getTaskLogs, executeFollowUpTask };
